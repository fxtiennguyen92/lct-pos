        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="w-full bg-white border border-gray-200 rounded shadow mb-6">
                <div class="bg-gray-50 divide-x divide-gray-200 rounded p-4">
                    <h5 class="text-left text-xs text-gray-500 uppercase">New Category</h5>
                </div>
                <div class="border-t border-gray-200">
                    <div class="bg-white rounded p-4">
                        <form action="{{ route('projects.product-categories.store', $project) }}" method="POST">
                            @csrf
                            <div class="m-4">
                            ...
                            <div class="m-4">
                                ...
                                <select name="parent_id" id="parent"
                                    ...
                                    @foreach ($project->productCategories as $parentCategory)
                                        <option value="{{ $parentCategory->id }}" @selected(old('parent_id') == $parentCategory->id)>
                                            {{ $parentCategory->name }}
                                        </option>
                                        @foreach ($parentCategory->children as $childCategory)
                                            <option value="{{ $childCategory->id }}" @selected(old('parent_id') == $childCategory->id)>
