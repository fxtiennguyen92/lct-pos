        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="w-full bg-white border border-gray-200 rounded shadow mb-6">
                <div class="bg-gray-50 divide-x divide-gray-200 rounded p-4">
                    <h5 class="text-left text-xs font-medium text-gray-500 uppercase">Product Attribute</h5>
                </div>
                <div class="border-t border-gray-200">
                    <div class="bg-white rounded p-4">
                        <form
                            action="{{ route('projects.product-attribute-sets.product-attributes.update', [$project, $productAttributeSet, $productAttribute]) }}"
                            method="POST">
                            @csrf
                            @method('PUT')
                            <div class="m-4">
                                ...
                                <input type="text" name="title" id="title"
                                    ...
                                    maxlength="150" value="{{ old('title', $productAttribute->title) }}" required>
                                @error('title')
                            ...
                            <div class="m-4">
                                ...
                                <input type="number" name="priority" id="priority"
                                    ...
                                    value="{{ old('priority', $productAttribute->priority) }}" required>
                                @error('priority')
                                    <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="m-4">
                                ...
                                <select name="status" id="status"
                                    ...
                                    @foreach (\App\StatusEnum::cases() as $status)
                                        <option value="{{ $status }}" @selected(old('status', $productAttribute->status) == $status->value)>
                                            {{ $status->label() }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <label id="defaultLabel" class="m-4 inline-flex items-center cursor-pointer">
                                <input type="checkbox" class="sr-only peer" name="default_flg" value="1"
                                    @checked(old('default_flg', $productAttribute->default_flg) == '1') />
                                <div
